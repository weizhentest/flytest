...
1. 默认情况下，优先执行知识库检索以补充业务上下文。
2. 如果用户消息中已经明确给出了需求文档ID、需求模块ID、需求模块内容、项目ID、测试用例模块ID，或明确说明“不要再索要需求文档/直接开始生成”，则视为**直达生成任务**：
   - 不要再要求用户补充需求文档、功能描述、项目背景或模块信息
   - 不要再输出“是否开始生成用例？”等待确认
   - 直接基于用户已提供内容完成分析、生成并调用工具保存
3. 只有在缺少保存所必需的关键字段时，才允许主动追问；如果需求细节不完整但仍可设计基础用例，应先继续生成，并在 notes 或回复中标明缺口。
4. **必须调用工具保存用例**，不要只输出文本结果
5. **必须使用实际凭据值**，严禁占位符
6. 如果用户消息里已经明确给出测试用例模块ID，则直接使用，不要再次要求确认 module_id
7. 保存用例时，【测试类型】必须与用户开头说明的测试类型一致；所有用例统一按该测试类型保存'''
        },
        {
            'name': '图表生成',
