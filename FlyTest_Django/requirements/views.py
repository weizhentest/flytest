        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    if direct_review:
                        # 直接评审允许从 uploaded / processing 等状态进入，这里只确保不是卡在 failed。
                        if document.status == "failed":
                            document.status = "uploaded"
                            document.save()
                    else:
                        # 模块评审的服务层要求文档至少处于 ready_for_review / reviewing。
                        if document.status not in ["ready_for_review", "reviewing"]:
                            document.status = "ready_for_review"
                            document.save()
                    ...
                    return Response(
                        {
                            "message": "Redis/Celery 不可用，评审已自动切换为同步执行并完成",
                            "review_type": "直接评审" if direct_review else "模块评审",
                            "direct_review": direct_review,
                    ...
                    document.save()
                    logger.error(
                        "启动评审失败：异步投递和同步兜底都失败。dispatch_error=%s, fallback_error=%s",
                        dispatch_error,
                        fallback_error,
                        exc_info=True,
                    )
                    return Response(
                        {
                            "error": (
                                "启动评审失败：Redis/Celery 不可用，且同步评审执行也失败。"
                                f" 详细信息：{fallback_error}"
                            )
                        },
                        status=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    )
            ...
            document.status = "failed"
            document.save()
            logger.error(f"启动评审失败: {e}", exc_info=True)
            return Response(
